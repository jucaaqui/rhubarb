...
from rhubarb import glsl
import __main__

class Shader:
    def __init__(self, source_code: str, num_invocations: tuple) -> None:
        """creates a GL compute shader which can be used to process data"""
        if type(num_invocations) == int: num_invocations = (num_invocations,)
        ...
        main = open(__main__.__file__).read()
        ...
        for i in range(num_includes):
            ...
            glsl_source = open(glsl.__file__).read()
        ...
        for uniform in Uniform.uniforms: header += "uniform %s %s;\n" % (uniform.glsl_type, uniform.name)
        ...
        if not ok: 
            sys.exit(glGetShaderInfoLog(shader).decode("utf-8"))
        ...
        texture_uniforms = [
            Uniform("int", Texture.textures[i].name, Texture.textures[i].unit, False) 
        ...
        size_uniforms = [
            Uniform(
                { 1: "int", 2: "ivec2", 3: "ivec3" }[len(Texture.textures[i].size)], 
                Texture.textures[i].name + "_size", 
                Texture.textures[i].size[0] if len(Texture.textures[i].size) == 1 else Texture.textures[i].size,
                False
            )
            for i in range(len(Texture.textures))]
        ...
        sampler_uniforms = [
            Uniform("int", Texture.textures[i].name + "_sampler", Texture.textures[i].unit, False)
            for i in range(len(Texture.textures))]
...
class Uniform:
    ...
    def __init__(self, glsl_type: str, name: str, val: Any, is_public: bool=True) -> None:
        self.name = name
        ...
        if is_public: Uniform.uniforms.append(self)

    def init(self, shader: Shader) -> None:
