...
class Texture:
    ...
    def __init__(self, name: str, size: tuple, data: bytes = None, fmt = "RGB"):
        ...
        self.size = size
        self.target = { 1: GL_TEXTURE_1D, 2: GL_TEXTURE_2D, 3: GL_TEXTURE_3D }[len(size)]
        self.upload_fun = { 1: glTexImage1D, 2: glTexImage2D, 3: glTexImage3D }[len(self.size)]
    ...
    def get_data(self, fmt = "RGB") -> bytes:
        ...
        glBindTexture(Texture.textures[-1].target, Texture.textures[-1].id)
